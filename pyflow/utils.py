...
def generate_method(cls: type):
    ...
    Wrap
        class A():
            def __init__(self, a: int, b: str = "x", **kwargs):
                pass
    into a function:
        def a(self, a: int, b: str = "x", **kwargs):
            return A(a=a, b=b, **kwargs)(self)
    ...
    pos_or_key = get_sig_param(sigs, inspect.Parameter.POSITIONAL_OR_KEYWORD)
    sigs = list(sigs.parameters.values())
    ...
    if pos_or_key:
        ...
        def inner(self, *args, **kwargs):
            return cls(*args, **kwargs)(self)
    else:
        sigs.insert(1, ARGS_SIG)
        sigs = inspect.Signature(sigs, return_annotation=OUTPUT_ANNOTATION)
